...
const Emergency = () => {
  ...
  const data = [
    ...
    { heading: 'Madhya Pradesh', content: ['AddressMadhya Pradesh State Disaster Management Authority Paryavaran Parisar E-5, Arera Colony, Bhopal 462016 Phone+(91) 0755-2446136/ 137/ 138', 'Emailmpstatecoord.sdma@mp.gov.in'] },
        ...
        <div className="hovered-content">
          {rowData.content.map((line, i) => (
            <div key={i} className="content-line">
              {line.includes('Phone') || line.includes('Ph') || line.includes('Contact No') ? <FaPhone /> : null}
              {line.includes('Fax') ? <FaFax /> : null}
              {line.includes('Email') || line.includes('E-Mail') ? <FaEnvelope /> : null}
              {line.includes('Address') || line.includes('P.O') || line.includes('Campus') ? <FaMapMarkerAlt /> : null}
              {line}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="emergency-container">
      ...
      <div className="marquee-container">
        <marquee className="marquee" scrollamount="10" bgcolor="#F9DEC9" direction="left" loop="">
      ...
      <div className="contact-details">
        <center>
          ...
          <p><FaPhone /> TELE/FAX NO. - 011-23438091, 011-23438136</p>
          <p><FaEnvelope /> EMAIL ID - hq.ndrf@nic.in</p>
          <h3>RECEPTION / EXCHANGE DETAILS</h3>
          <p><FaPhone /> NO. - 011-23438017, 011-23438019</p>
        </center>
      </div>
      <div className="states-section">
        ...
        <div className="cards-container">
          {data.map((row, index) => (
            ...
            >
              <div className="card-header">{row.heading}</div>
              <div className="card-content">
                {row.content.map((line, i) => (
